    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        ...
        <Card className="p-4 bg-indigo-50/50 border border-indigo-100">
          <span className="text-[10px] font-extrabold text-[#2012ad] uppercase tracking-wider">Total Net Payable</span>
          <p className="text-xl font-extrabold text-[#2012ad] mt-1">₹{totalNet.toLocaleString()}</p>
        </Card>
      </div>
              ...
              <tbody className="divide-y divide-slate-100 text-slate-700 font-semibold">
                {periods.map((p) => (
                  <tr key={p.id} className="hover:bg-slate-50/60 transition-all">
                    ...
                    <td className="py-3 px-4 font-extrabold text-[#2012ad]">₹{(p.total_net || 0).toLocaleString()}</td>
                      ...
                      <span
                        className={`px-2 py-0.5 rounded-full text-[10px] font-extrabold ${
                          p.status === 'LOCKED'
                            ...
                            : p.status === 'CALCULATED'
                            ? 'bg-indigo-50 text-[#2012ad] border border-indigo-200'
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 border-b border-slate-100 pb-3">
            <div>
              <h3 className="text-base font-bold text-slate-900">
                Payroll Period: <span className="text-[#2012ad]">{selectedPeriod?.name || 'Select Period'}</span>
              </h3>
              ...
              <tbody className="divide-y divide-slate-100 text-slate-700 font-semibold">
                {payrollRecords.map((r) => (
                  <tr key={r.id} className="hover:bg-slate-50/60 transition-all">
                    ...
                    <td className="py-3 px-4 font-extrabold text-base text-[#2012ad]">₹{r.net_salary.toLocaleString()}</td>
                    <td className="py-3 px-4 text-right">
                      <Button
                        size="sm"
                        variant="outline"
              ...
              <tbody className="divide-y divide-slate-100 text-slate-700 font-semibold">
                {advances.map((a) => (
                  <tr key={a.id} className="hover:bg-slate-50/60 transition-all">
                    <td className="py-3 px-4 font-mono font-bold text-[#2012ad]">{a.staff_code}</td>
